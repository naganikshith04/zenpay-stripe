...
from db.models import Customer as CustomerDB
...
def create_customer(
    ...
    db_customer = (
        db.query(Customer)
        .filter(Customer.user_id == current_user.id, Customer.id == customer.id)
        .first()
    )
    ...
    if db_customer:
        ...
        if customer.metadata is not None:
            db_customer.metadata_json = (
                metadata_str  # Changed from metadata to metadata_json
            )
    else:
        # Create new customer
    ...
    return {
        ...
        "metadata": (
            json.loads(db_customer.metadata_json) if db_customer.metadata_json else {}
        ),
        "created_at": db_customer.created_at.isoformat(),  # Optional: ensures clean string
    }
...
def get_customer(
    ...
    customer = (
        db.query(Customer)
        .filter(Customer.user_id == current_user.id, Customer.id == customer_id)
        .first()
    )
    ...
    return {
        ...
        "metadata": (
            json.loads(customer.metadata_json) if customer.metadata_json else {}
        ),  # Changed from metadata to metadata_json
        "created_at": customer.created_at.isoformat()
,
    }
